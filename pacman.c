...
clock_t t = 0;  //Time taken for ai cauculatation
void print_usage(){
...
void ExitProgram(const char *message) {
    out_put();                      //Output informations to file
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
             ...
             */
             move_t selected_move = get_next_move( current_state, budget,
                 propagation, ai_stats, &total_max_depth, &total_generated,
                 &total_expanded, &t);
            /**
...
void out_put(){
    ...
    //Change it to unit of second
    double time_taken = ((double)t) / CLOCKS_PER_SEC;
    int expanded_per_second = (int) (total_expanded / time_taken);
    FILE *write_file;
    char *file_name = "output.txt";
    write_file = fopen(file_name, "w+");
	assert(write_file != NULL);
    //Output needed informations
    if (propagation == max) {
        fprintf(write_file, "Propagation = Max\n");
    } else if (propagation == avg) {
        fprintf(write_file, "Propagation = Average\n");
    }
    fprintf(write_file, "Budget = %d\n", budget);
    fprintf(write_file, "MaxDepth = %d\n", total_max_depth);
    fprintf(write_file, "TotalGenerated = %d\n", total_generated);
    fprintf(write_file, "TotalExpanded = %d\n", total_expanded);
    fprintf(write_file, "Time = %.2f seconds\n", time_taken);
    fprintf(write_file, "Expanded/Second = %d\n", expanded_per_second);
    fprintf(write_file, "Score = %d\n", Points);
    fclose(write_file);
}
